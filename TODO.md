...
1. After scanning a completed download's storage path, run FFmpeg-based pHash generation on each video file (`.mkv`, `.mp4`, etc.) — see the OSHash implementation in `src/pmm.Api/Features/DownloadLogs/OsHash.cs` for reference.
